...
export default function ClientPortal() {
  ...
  useEffect(() => {
    ...
    getSchwabPositions(user.id, portfolio.schwab_account_hash)
      ...
      .catch(err => {
        console.error('[ClientPortal] Schwab fetch failed:', err.message);
        if (!cancelled) setSchwabData(null);
      });
    return () => { cancelled = true; };
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      ...
      <main className="max-w-screen-xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="flex items-center justify-between mb-1">
          <span className="text-xs text-slate-400">Performance updates every 5 minutes during market hours</span>
          <button
